...
fn check_safe_report(line: String) -> bool {
    ...
    for (index, report) in reports.iter().enumerate() {
        let current: i32 = report.parse().unwrap();
...
fn main() -> io::Result<()> {
    ...
    for line in reader.lines() {
        match line {
            Ok(line) => {
                let initial_safe = check_safe_report(line.clone());
                if initial_safe {
                    safe_count += 1;
                } else {
                    ...
                    for i in 0..tokens.len() {
                        ...
                        modified_tokens.remove(i);
                        let modified_line = modified_tokens.join(" ");

                        let result = check_safe_report(modified_line.clone());
                        if result {
                            made_safe = true;
                            break;
                        }
